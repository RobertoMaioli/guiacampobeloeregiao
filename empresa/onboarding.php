 ...
 *
 * Fluxo:
 *   Step 0 — Dados básicos (nome, categoria, descrição)
 *   Step 1 — Endereço + contato (CEP com autocomplete ViaCEP)
 *   Step 2 — Horários de funcionamento
 *   Step 3 — Fotos + preview + envio para análise
 *
 * Salvamento automático via AJAX a cada 1,5s após última edição.
 * Endpoint: /empresa/actions/salvar.php
 */
...
// Sessão ativa mas usuário excluído do banco → desloga
if (!$usuario) {
    UserAuth::logout();
...
// Redireciona se o cadastro já passou desta etapa
...
$lugar = $lugar_id
    ? DB::row('SELECT * FROM lugares WHERE id=? AND empresa_id=?', [$lugar_id, $empresa_id])
    : null;
...
$fotos_atuais = $lugar_id
    ? DB::query('SELECT id,url FROM fotos WHERE lugar_id=? ORDER BY ordem', [$lugar_id])
    : [];
...
// Helper: valor do campo (POST > banco > default)
...
/* ── CEP: feedback visual de carregamento ── */
#f-cep.loading { opacity: .4; pointer-events: none }
</style>
</head>
<body>
<div class="onb-wrap">
...
<div class="onb-panel" id="panel-1">
  ...
  <!-- Localização — CEP preenche endereço e bairro via ViaCEP -->
  <div class="onb-card">
    ...
    <div class="mb-3">
      ...
      <label class="olbl">CEP</label>
        <input type="text" class="gcb-field" id="f-cep"
               ...
               value="<?= $v('cep') ?>"
               oninput="autoSave(); onCepInput(this.value)"
  ...
  <div class="onb-card">
    ...
    <!-- Instagram (bloqueado no essencial) -->
...
<div class="onb-panel" id="panel-2">
  <div class="onb-card">
    ...
    <!-- Atalho: aplica horário de Seg a uma vez para todos os dias úteis -->
    ...
    <!-- Loop dos 7 dias (0=Domingo … 6=Sábado) -->
    <?php for ($d = 0; $d <= 6; $d++):
...
<!-- ══ STEP 3 — FOTOS + PREVIEW + ENVIO ══════════════════════════════ -->
<div class="onb-panel" id="panel-3">

  <!-- Upload de fotos (disponível apenas nos planos pagos) -->
  <div class="onb-card">
    <div class="card-ttl">
      <?= icon('grid', 17) ?> Fotos
      <span style="font-size:10px;color:var(--gcb-warmgray);font-weight:500">
        <?php if ($plan === 'essencial'): ?>não disponível
        <?php elseif ($plan === 'profissional'): ?>até 5
        <?php else: ?>ilimitadas
        <?php endif; ?>
      </span>
    </div>

    <!-- Thumbnails já existentes no banco -->
    ...
    <?php else: ?>
    <!-- Mensagem de upgrade para o plano Profissional -->
    <div style="padding:16px;background:var(--gcb-offwhite);border-radius:12px;border:1.5px dashed rgba(61,71,51,.15);text-align:center">
      <p style="font-size:13px;color:var(--gcb-warmgray);margin:0">
        📷 Fotos disponíveis a partir do plano
        <strong style="color:var(--gcb-green-dark)">Profissional</strong>.
        <a href="/pages/anuncie.php" style="color:var(--gcb-gold);font-weight:700">Ver planos</a>
      </p>
    </div>
    <?php endif; ?>

    <!-- Preview das fotos selecionadas (renderizado pelo JS) -->
    <div class="foto-grid" id="foto-preview"></div>
  </div>

  <!-- Card de preview da página pública -->
  <div class="onb-card">
    ...
    <div class="prev-card">
      ...
      <div class="prev-body">
        <div style="font-size:9px;font-weight:800;letter-spacing:.14em;text-transform:uppercase;color:var(--gcb-gold);margin-bottom:3px"
             id="prev-cat">Categoria</div>
        <div style="font-size:16px;font-weight:800;color:var(--gcb-green-dark);margin-bottom:4px"
             id="prev-nome">Nome da empresa</div>
  ...
  <div class="onb-card">
    ...
    <div class="res-row">
      ...
      <span class="res-val">
        <span class="plan-tag"><?= Sanitize::html($plan_labels[$plan] ?? 'Essencial') ?></span>
      </span>
    </div>
    <div class="res-row">
...
// ── Estado local ─────────────────────────────────────────────────────
let cur       = 0;           // step atual
let lugarId   = LUGAR_DB;    // id do lugar (atualizado após 1º save)
let fotoFiles = [];          // arquivos selecionados ainda não enviados
let debouce   = null;        // timer do autoSave
...
function goStep(n) {
  ...
  // Esconde todos os panels e mostra o alvo
  ...
  document.querySelectorAll('.onb-step').forEach((t, i) => {
    ...
    document.getElementById('snum-' + i).innerHTML = i < n ? '✓' : i + 1;
  });

  cur = n;
  window.scrollTo({ top: 0, behavior: 'smooth' });

  // Atualiza preview ao chegar no step 3
...
function validar(step) {
  if (step === 0) {
    if (!document.getElementById('f-nome').value.trim()) {
      alert('Informe o nome da empresa.'); return false;
    }
    if (!document.getElementById('f-categoria_id').value) {
...
//  Debounce de 1,5s — chama salvar() após última edição
// ════════════════════════════════════════════════════════════════════════
function autoSave() {
...
async function salvar() {
  ...
  try {
    ...
    if (d.ok) { lugarId = d.lugar_id; setDot('saved'); }
    else      { setDot(''); }
  } catch { setDot(''); }
}

// Helper: pega valor de um campo pelo id
function g(id) {
...
// ════════════════════════════════════════════════════════════════════════
//  AUTOCOMPLETE DE CEP — ViaCEP (https://viacep.com.br)
//  Preenche: logradouro (se vazio) e bairro
//  Acionado: ao digitar 8 dígitos OU ao sair do campo (blur)
// ════════════════════════════════════════════════════════════════════════

// Chamado pelo oninput do campo CEP
...
async function buscarCep(cep) {
  ...
  try {
    ...
    // Preenche endereço somente se estiver vazio
    // (preserva o número que o usuário já pode ter digitado)
    const elEnd = document.getElementById('f-endereco');
    if (elEnd && !elEnd.value.trim()) {
      elEnd.value = d.logradouro || '';
    }

    // Bairro: sempre preenche (vem direto do CEP)
    const elBairro = document.getElementById('f-bairro');
    if (elBairro) elBairro.value = d.bairro || '';

    // Dispara autoSave para persistir os dados preenchidos
    autoSave();

  } catch (e) {
    // Falha silenciosa — usuário preenche manualmente
  } finally {
    elCep.classList.remove('loading');
  }
}
...
// Habilita/desabilita inputs de horário conforme checkboxes
function togH(d) {
...
// Copia horário da Segunda (dia 1) para todos os dias úteis (1–5)
...
// Salva horários via AJAX (endpoint separado)
async function saveH() {
  ...
  for (let d = 0; d <= 6; d++) {
    h.push({
      dia:      d,
      fechado:  document.getElementById('hf' + d).checked ? 1 : 0,
      dia_todo: document.getElementById('hd' + d).checked ? 1 : 0,
      abre:     document.getElementById('ha' + d).value,
      fecha:    document.getElementById('hc' + d).value,
    });
  }
  await fetch('/empresa/actions/horarios.php', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ _token: CSRF, lugar_id: lugarId, horarios: h })
  });
}
...
// Renderiza thumbnails das fotos locais (antes do upload)
  ...
  fotoFiles.forEach((f, i) => {
    ...
    reader.onload = e => {
      ...
      div.innerHTML = `<img src="${e.target.result}">
                       <button class="frem" onclick="remFoto(${i})">✕</button>`;
      grid.appendChild(div);
      // Primeira foto atualiza o preview do card
...
async function uploadFotos(files) {
  // Garante que o lugar já foi criado antes de enviar
...
async function submeter() {
  // Valida steps obrigatórios antes de enviar
  if (!validar(0) || !validar(1)) { goStep(0); return; }

  // Garante que há um lugar salvo
  if (!lugarId) await salvar();
  ...
  try {
    const r = await fetch('/empresa/actions/submeter.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ _token: CSRF, empresa_id: EMP_ID, lugar_id: lugarId })
    });
    const d = await r.json();
    if (d.ok) {
